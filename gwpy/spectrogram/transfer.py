# -*- coding: utf-8 -*-
# Copyright (C) Derek Davis and Duncan Macleod (2018)
#
...
time-frequency transfer function spectrogram from a pair of time-series.
"""
...
def _from_timeseries(ts1, ts2, stride, fftlength=None, overlap=None,
                     window=None, **kwargs):
    """Generate a time-frequency transfer function
    ...
    out = Spectrogram(zeros((nsteps, nfreqs)), epoch=ts1.epoch, dt=stride,
                      f0=0, df=1/fftlength, copy=True, unit='transfer function')
...
def from_timeseries(ts1, ts2, stride, fftlength=None, overlap=None,
    ...
    """Calculate the transfer function `Spectrogram` between two `TimeSeries`.
    ...
    for i in range(nproc):
        process = Process(target=_tfgram,
